...
| `functions.php`                                                   | Theme supports, Carbon Fields boot, `admin_text()`, `section_field()` and the section block helpers, `theme_icon()` and `theme_icon_options()` (the options of an icon picker field), `is_rising()` and `trend_icon()` (a chart and its icon that follow whether a number went up), `story_curve()` (the curve of a case study card), `text_lines()` (a textarea read as a list, one item per line), palette, performance filters.                                                                |
...
| `template-parts/sections/home/*.php`                              | The 18 home sections. `home-banner.php`, `home-audience.php` (Who it's for), `home-benefits.php` (Benefits), `home-program.php` (Program), `home-proof.php` (Social proof) and `home-pricing.php` (Pricing) are built. The other twelve are empty stubs waiting for their fields. |
...
`home_banner`, `home_audience`, `home_benefits`, `home_program`, `home_proof`, `home_pricing`, `home_services`, `home_about`, `home_team`, `home_cta`, `home_results`, `home_testimonials`, `home_social`, `home_news`, `home_contact`, `home_mission`, `home_portfolio`, `home_faq`, `about_rev`

**With fields (a Carbon Fields block in `includes/section-blocks.php`):** `home_banner`, `home_audience`, `home_benefits`, `home_program`, `home_proof`, `home_pricing`, `about_rev`.

**Without fields yet:** the other twelve. Their template is an empty stub, and in the editor their block shows *Add the required fields to the theme to display and edit them in the WordPress admin and on the website.* A section stops being one of these the moment a `Block::make()` exists for its tag: the inserter, the block name (`yk4/home-x` becomes `carbon-fields/home-x`) and the single-instance rule follow the block registry, nothing else is switched by hand. A page that already holds the old placeholder block of a section that then gets fields shows it as an unsupported block until it is replaced, though the front end keeps rendering the shortcode.
...
A block section is the same section file plus three things: a Carbon Fields `Block`, a row in the inserter list and a render through the shortcode. `home_banner` is the reference, `about_rev` the second one (tabs, a select, stats) `home_audience` the third (two repeaters, an icon select, a card grid that sizes itself to its cards) `home_benefits` the fourth (a bento: two featured cards around a repeater of tiles, a decorative chart whose values count up and whose curve and trend icon turn upward when the end value is higher than the start, a checklist) and `home_program` the fifth (two halves in a 12 column grid that stack in a different order on a phone, a photo under a frosted label, a repeater of label and value facts, numbered step cards whose check lines are one textarea with a line per item) `home_proof` the sixth (a rating, a row of count-up numbers that sizes itself to how many there are, a video card whose play link opens a popup from a YouTube link or a media file, a quote, a chat and reviews in a row that stretches over the cards it has, and a slider of case study cards on a native scroller with arrows, a position bar and dots, whose cards chart a before and an after and turn the curve upward when the after is higher, then a strip of logos) and `home_pricing` the seventh (a centred heading over a row of plans that sizes itself to how many there are, each plan a row of a repeater with a Highlight checkbox that turns it into a white card standing above the others, a badge on its top edge, a price with its period and note, a full width button and a check list that is one textarea with a line per item, then a footnote line).
...
5. **Fields use the `crb_<section>_<name>` names** like every other section. Repeaters are `complex` fields: give every default row a unique `_id` (the block editor removes rows by it), call `->set_header_template()` after `add_fields()`, and never name a subfield `value` or `_type` (see the gotcha under *Fields*).
...
7. **Images in a block go through `the_content` before shortcodes do**, so `filter_url()` turns their `src` into `data-url` for the lazy loader. Mark an image that is above the fold `loading="eager"` (add `fetchpriority="high"` to the largest) and it keeps its `src`. The filter rewrites the `src` of any tag, not only images, and the lazy loader only restores `img` and `iframe`, so a `<video>`, or an `iframe` that must not load until it is asked for (a video popup), is never written into the markup with a `src`: the section's script creates it when it is needed (see *JavaScript*).
...
type       --xxl display (hero H1) 104   --xl h1 80   --lg h2 68   --lg-narrow h2 in a narrow column 60   --md-lg h3 of a wide row 48   --md h3 32   --sm h4 22   --stat card numerals 40   --numeral step ordinal 56   --figure big results figures 84   --stat-lg before/after numerals 44   --quote pull quote 29   --quote-mark its opening mark 120   --lead 20
           --card-title card h3 28   --card-title-lg featured card h3 34   --card-text card copy 17   --card-text-lg review copy 19   --card-note card foot line 15   --plan-title pricing plan h3 32   --price plan price 72
...
- **A block's keywords are capped at three** (`set_keywords()` slices the list), so a Ukrainian entry that appends its words to the English three, as the first blocks do, never reaches the inserter search. Write the Ukrainian value as its own three words (`home_pricing` uses `ціни,тарифи,pricing`).
...
A file from the media library is a `file` field read the same way, `Field::make('file', ...)->set_type(array('video'))->set_value_type('url')` (the type limits what the media picker offers). `home_proof` gives a video two sources, a YouTube link in a text field and such a file, and plays the file when both are filled. A repeater subfield may be an image too (its logos), with the same `set_value_type('url')`.
...
A yes/no per row is a `checkbox` subfield (the Highlight of a `home_pricing` plan). A block hands the template its raw saved value, which the editor stores as a boolean (its default row says `true`), so test it with `!empty($row['featured'])` and never compare it to the option value `'yes'`, which is what a checkbox outside a block would give back.

A short list inside a repeater row (the check lines of a step) is a `textarea` with one item per line, not a repeater nested in the repeater: the client types three lines instead of opening three accordions, and no default row needs an `_id`. Read it with `text_lines($row['points'])`, which trims, skips blank lines and splits on CR and LF only, because a `\R` without the `u` flag would cut a Cyrillic letter in half (the byte 0x85 ends х).
...
Gotcha: a `complex` subfield, in a block or in Theme Options, must not be called `value` or `_type`. Carbon Fields reserves both. It records *"value" is a reserved keyword for Complex fields*, which wp-admin shows as the red *Your site seems to be slightly misconfigured* notice, and it leaves the whole group out of the field, so the block crashes in the editor (*This block has encountered an error and cannot be previewed*). The front end keeps rendering, because it only reads values and defaults, so nothing on the page gives it away and a render test never sees it. In Theme Options the name is also a collision on its own: those rows are stored as `_field|subfield|row|index|value`, so the row would save with an empty subfield slot and read back as an empty array, silently. Name the subfield something else (`detail`, `amount`, `answer`) and keep the label. After adding or changing a repeater, boot the theme with `WP_DEBUG` off and check that `Carbon_Fields\Exception\Incorrect_Syntax_Exception::$errors` is empty (with `WP_DEBUG` on it throws instead).
...
A count-up (the hero cards, the `about_rev` stats, the `home_benefits` chart) follows one pattern: the markup holds the final value, so the Preview (which runs no scripts) and a visitor without JS see the real number; the section's script counts it up once, the first time the element is seen, and keeps whatever surrounds the number (a unit, a sign). A new one should also do nothing under `prefers-reduced-motion`, treat a decimal comma like a point and end on the author's exact text, as `home_benefits` does. A price is not a statistic and never counts up (`home_pricing` has no script): a visitor comparing plans is reading the number, and one that moves while they look reads as a different price. `home_proof` is the version for a row of statistics: each number starts when it is seen, and a space or a comma before exactly three digits is read as thousands (`1,200`, `1 200`, `12,500+`, as `first_number()` reads them) and stays in the text while it counts.

A full screen overlay (the `home_proof` video popup is the reference) is written inside the section file but moved to the end of `<body>` by its own script, so no ancestor can clip a `position: fixed` element. Its CSS is therefore keyed off its own top level class rather than the section class, and it reads its content out of the DOM, the play link's own `href`, instead of carrying invented `data-` attributes. The trigger is a real link, so it still works without the script (it opens the video in a new tab), and the popup opens only for an address it can play, a YouTube page (any of its link forms, with a start time) or a `.mp4`, `.webm`, `.ogg`, `.mov` or `.m4v` file, and leaves every other address to the link. The player is made when the popup opens and removed when it closes, which is what stops it, and Escape, a click outside it and the close button all close it and hand the focus back.
